...
export default function ReadingActivity() {
  ...
  useEffect(() => {
    ...
    fetch("https://levelup-jpnn.onrender.com/api/test-result", {
      ...
      body: JSON.stringify({
        ...
        isCompleted: true,
        sessionKey: `reading-${learnerId}-${startedAt}`,
      }),
    })
      .then((res) => res.json())
  ...
  const handleDrop = (e, targetSlotIndex) => {
    ...
    try {
      ...
      if (isSlot) {
        ...
        if (currentWord) setTiles((t) => [...t, currentWord]);
      } else {
        const newSlots = [...slots];
  ...
  const checkAnswer = () => {
    ...
    if (correct) {
      ...
      speak(`The correct sentence is: ${current.correctOrder}`);
    }
  };
  ...
  if (sessionDone) {
    ...
    if (!p.reading?.sessions?.length || p.reading.sessions.length <= 1) {
    ...
    return (
      <div style={{ padding: 32 }}>
        ...
        <button onClick={() => navigate("/reading", { replace: true })}>
          Back to Reading
        </button>
      </div>
    );
