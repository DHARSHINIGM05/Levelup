...
export default function WritingActivity() {
  ...
  const difficulty =
    location.state?.difficulty || getModuleDifficulty("writing");
  ...
  useEffect(() => {
    ...
    const list =
      valuesList.length >= SESSION_SIZE
        ? valuesList
        : getWritingContentByGrade(level, grade);
    setPool(list);
  ...
  useEffect(() => {
    if (!sessionDone || resultSubmitted) return;
  ...
  const handleOptionClick = (optionId) => {
    ...
    if (correct) {
      ...
      if (nextIndex >= sessionItems.length) {
        ...
        const p = getProgress();
        if (!p.writing?.sessions?.length || p.writing.sessions.length <= 1)
          unlockBadge(p, "first_writing");
    ...
    return (
      <div style={styles.card}>
        ...
        <p style={{ fontSize: "1.1rem", marginBottom: 24 }}>
          You got {correctCount} out of {sessionItems.length} correct. Progress
          saved.
        </p>
        <button
          type="button"
          ...
          onClick={() => {
            ...
            setSessionItems(
              shuffled.slice(0, Math.min(SESSION_SIZE, shuffled.length)),
            );
            setFeedback(null);
          }}
        >
  ...
  return (
    <div style={styles.card}>
      <p style={{ fontSize: "1rem", color: "#555", marginBottom: 16 }}>
        Question {index + 1} of {sessionItems.length}
      </p>
      <button
        type="button"
        style={styles.replay}
        onClick={() => {
          speak(current.instruction_tts);
          setTimeout(() => speak(current.text_for_tts), 2200);
        }}
      >
        🔄 Hear word again
      </button>
      <h2 style={styles.title}>
        Listen to the word. Click the correct spelling.
      </h2>
      <div>
