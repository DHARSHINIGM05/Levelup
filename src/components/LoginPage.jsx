...
const LoginPage = () => {
  ...
  const {
    registeredChild: globalChild,
    setRegisteredChild: setGlobalRegisteredChild,
  } = useApp();
  ...
  useEffect(() => {
    if (mode === "parent") {
      ...
      const classLabel =
        child.classType === "primary" ? "primary" : "secondary";
      speak(
        `Hello ${child.childName}. You are logging in as ${classLabel} class ${child.grade} student. Click one of your pictures to log in.`,
      );
  ...
  const handleLoginIconToggle = (iconId, label) => {
    setSelectedLoginIconIds((prev) => {
      ...
      speak(
        `Picture ${label} ${prev.includes(iconId) ? "removed" : "selected"}.`,
      );
      playConfirmationTone();
      return next;
    });
  ...
  const handleInattentive = useCallback(() => {
    ...
    const child =
      selectedChildIndex !== null
        ? registeredChildren[selectedChildIndex]
        : null;
    ...
    >
            ...
            <form onSubmit={handleRegister} style={styles.form}>
                ...
                <div style={styles.avatarGrid}>
                  {AVATARS.map((avatar) => (
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        handleAvatarSelect(avatar.id, avatar.label)
                      }
                      style={{
                        ...styles.avatarButton,
              ...
              <section aria-label="Select class type" style={styles.formGroup}>
                ...
                <p style={styles.helperText}>
                  Is your child in primary (1–5) or secondary (6–12)?
                </p>
                <div style={styles.classTypeRow}>
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setGrade(null);
                      speak(
                        "Secondary class selected. Now choose grade 6 to 12.",
                      );
                      playConfirmationTone();
                    }}
                    style={{
                  ...
                  {LOGIN_ICON_CHOICES.map((icon) => {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        onClick={() =>
                          handleLoginIconToggle(icon.id, icon.label)
                        }
                        style={{
                          ...styles.iconButton,
          ...
          <section style={styles.card} aria-label="Choose your profile">
            ...
            {registeredChildren.length === 0 ? (
              <p style={styles.helperText}>
                ...
                device. Please ask them to tap the &quot;Parent / Mentor&quot;
                button.
              </p>
            ) : (
              <div style={styles.avatarGrid}>
          ...
          <section style={styles.card} aria-label="Child login">
            <div>
              <header style={styles.childHeader}>
                ...
                <div>
                  <h2 style={styles.sectionTitle}>
                    Hello {registeredChildren[selectedChildIndex].childName}!
                  </h2>
                  <p style={styles.helperText}>
                    You are logging in as{" "}
                    {registeredChildren[selectedChildIndex].classType ===
                    "primary"
                      ? "primary"
                      : "secondary"}{" "}
                    class {registeredChildren[selectedChildIndex].grade}{" "}
                    student.
                  </p>
                  <p style={styles.helperText}>
              ...
              >
                {registeredChildren[selectedChildIndex].loginIcons.map(
                  (icon) => (
                    <button
                      key={icon.id}
                      type="button"
      ...
      <aside style={styles.cameraPanel} aria-label="Camera monitoring area">
        <section style={styles.cameraCard}>
          ...
          <p style={styles.helperText}>
            The camera gently watches to see if you are looking at the screen
            and staying engaged. This helps your grown‑ups understand how you
            are doing.
          </p>
          <CameraMonitor />
        </section>
...
const styles = {
  page: {
    ...
    background: "linear-gradient(135deg, #FFD6B3, #FFE0CC)",
    fontFamily:
